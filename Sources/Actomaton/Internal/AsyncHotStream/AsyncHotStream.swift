// See https://github.com/inamiy/AsyncHotStream
...
extension AsyncStream
{
    ...
    static func makeHotStream() -> (Self, Self.Continuation)
    {
    ...
    /// 1. `AsyncStream` as `for await`-able receiver
    /// 2. `Continuation` as `yield`-able sender
    static func makeNonTerminalHotStream() -> (Self, (Element) -> Self.Continuation.YieldResult)
    {
        let hot = AsyncHotStream<Element>()
